...
LOAD DATA  INFILE 'C:/ProgramData/MySQL/MySQL Server 8.3/Uploads/aisles.csv' INTO TABLE aisles
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 LINES;

LOAD DATA  INFILE 'C:/ProgramData/MySQL/MySQL Server 8.3/Uploads/departments.csv' INTO TABLE departments
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 LINES;


LOAD DATA  INFILE 'C:/ProgramData/MySQL/MySQL Server 8.3/Uploads/orders_small_version.csv' INTO TABLE orders
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 LINES;

ALTER TABLE product MODIFY COLUMN name VARCHAR(5000);
LOAD DATA  INFILE 'C:/ProgramData/MySQL/MySQL Server 8.3/Uploads/products.csv'
 INTO TABLE product
FIELDS TERMINATED BY ',' ENCLOSED BY '"'
ESCAPED BY '\\' LINES TERMINATED BY '\n' IGNORE 1 LINES;

SET FOREIGN_KEY_CHECKS=0;
LOAD DATA  INFILE 'C:/ProgramData/MySQL/MySQL Server 8.3/Uploads/order_products.csv' INTO TABLE order_product
FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 LINES;

SELECT DISTINCT order_id
FROM order_product
WHERE order_id NOT IN (SELECT id FROM orders);
UPDATE orders SET id = 2398795 WHERE id = 12;
UPDATE orders SET id = 2539329 WHERE id = 11;
...
FROM
	...
	@rank := IF(@current_day = day, @rank + 1, 1) AS rank_value,
	@current_day := day
	FROM
...
SELECT  aisle, day, id as product_id FROM
	...
	@rank := IF(@current_aisle=aisle AND @current_day = day, @rank + 1, 1) AS rank_value,
	@current_day := day,
    @current_aisle := aisle
    FROM
